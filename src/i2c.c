...
esp_err_t i2c_bus_probe_peripherals(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Optional Light Sensor (TSL2561) not found");
    } else {
...
{
    if (lux_value == NULL || raw_ch0 == NULL || raw_ch1 == NULL) {
        return ESP_ERR_INVALID_ARG;
    }

    // Force a continuous Power-On rewrite loop to ensure the chip's ADCs are active
    i2c_sensor_wakeup_light();
    ...
    // Combine raw registers and save to pointer addresses
    *raw_ch0 = (uint16_t)((ch0_data[1] << 8) | ch0_data[0]);
    *raw_ch1 = (uint16_t)((ch1_data[1] << 8) | ch1_data[0]);
    ...
    float ratio = (float)(*raw_ch1) / (float)(*raw_ch0);
    float lux = 0.0f;

    if (ratio <= TSL2561_RATIO_K1) {
        lux = (0.0304f * (*raw_ch0)) - (0.062f * (*raw_ch0) * powf(ratio, 1.4f));
    } else if (ratio <= TSL2561_RATIO_K2) {
        lux = (0.0224f * (*raw_ch0)) - (0.031f * (*raw_ch1));
    } else if (ratio <= TSL2561_RATIO_K3) {
        lux = (0.0128f * (*raw_ch0)) - (0.0153f * (*raw_ch1));
    } else if (ratio <= TSL2561_RATIO_K4) {
        lux = (0.00146f * (*raw_ch0)) - (0.00112f * (*raw_ch1));
    } else {
        lux = 0.0f; 
    }
